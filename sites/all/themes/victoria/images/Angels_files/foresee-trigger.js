...
function fsr$setAlive(){var A=new Date().getTime();document.cookie="fsr.a"+(FSR.cookie?"."+FSR.cookie:"")+"="+A+";path=/"+((FSR.site.domain)?";domain="+FSR.site.domain+";":";")
...
for(var F=0,H=A.length;F<H;F++){if(FSR.site[A[F]]){FSR[A[F]]=FSR.site[A[F]]}}if(FSR.site.cookie){FSR.cookie=FSR.site.cookie
}break}}if(B){break}}if(!window["fsr$timer"]){fsr$setAlive();window["fsr$timer"]=setInterval(fsr$setAlive,1000)
...
FSR.$P=function(){return FSR.properties};FSR.c=function(){return new FSR.Hash.Cookie(FSR.cookieName("fsr.s"),{path:"/",domain:FSR.site.domain||false})
};FSR.cookieName=function(A){return A+(FSR.cookie?"."+FSR.cookie:"")};FSR.log=function(D,C){if(!FSR.$P().events.enabled){return 
...
var Q=FSR.Cookie.read(FSR.cookieName("fsr.a"),{path:"/",domain:FSR.site.domain||false});if(!this.trackerRunning()){this.cancelAlive()
}if(!Q){fsr$dbug.log("Exit: Cookies are not enabled");this.v=0;return }var S=FSR.c();var K=FSR.$P();
var G=S.get("v");if(!FSR.$defined(G)){G=1;var U=FSR.Browser;fsr$dbug.log("browser: "+U.Type.name+" "+U.Type.version+" on "+U.Platform.os);
if(!FSR.SupportedPlatforms[U.Platform.name]){FSR.$S.message="Exit: Platform not surpported";G=0}if(FSR.UnsupportedBrowsers[U.Type.name]){if(U.Type.version<=FSR.UnsupportedBrowsers[U.Type.name]){FSR.$S.message="Exit: Browser not surpported";
G=0}}if(this.exclude()){FSR.$S.message="Exit: Met exclude criteria";G=0}var O=FSR.Cookie.read("fsr.o",{path:"/",domain:FSR.site.domain||false});
if(O){FSR.$S.message="Exit: Optout Cookie Found";G=0}var P=new FSR.Hash.Cookie(FSR.cookieName("fsr.r"),{path:"/",domain:FSR.site.domain||false});
var N;if(N=P.get("d")){FSR.$S.message="Persistent Cookie Found: "+N;G=-1}var T;if(K.altcookie&&K.altcookie.name){T=FSR.Cookie.read(K.altcookie.name,{path:K.altcookie.path||false,domain:K.altcookie.domain||false});
if(T){if(!K.altcookie.value||K.altcookie.value==T){FSR.$S.message="Alt persistent Cookie Found: "+T;
G=-1}}}var H=FSR.Cookie.read("fsr.pool",{path:"/",domain:FSR.site.domain||false});var M=(H==1)?100:FSR.$random(0,100);
if(G==1&&!(M<=this.pool())){FSR.$S.message="Not in pool: "+M;G=-2}S.set("v",G);var L;if(L=P.get("i")){var I=new Date();
if(I.getTime()<P.get("e")){FSR.rid=L}}if(!FSR.rid){if(K.events.enabled&&K.events.id){FSR.rid=FSR.generateid()
}}if(FSR.rid){S.set("rid",FSR.rid)}var A;if(A=P.get("s")){S.set("sd",A);S.set("lk",1)}var J=this;
this.dhtml_win=1;new FSR.Asset.javascript((FSR.js_files||FSR.files)+"foresee-dhtml-popup.js",{id:"foresee-dhtml-popup",onload:function(){J.dhtml_win=2
}});this.dhtml_css=1;new FSR.Asset.css((FSR.css_files||FSR.files)+K.invite.css);if(K.qualifier.css&&(K.qualifier.css!=K.invite.css)){new FSR.Asset.css((FSR.css_files||FSR.files)+K.qualifier.css)
}this.dhtml_css=2;if(document.referrer&&document.referrer!=""){if(K.meta.ref_url){S.set("ru",document.referrer)
}var E=document.referrer.match(/^(\w+\:\/\/)?(((\w+-?\w+\.?))+)\//);var C="";if(E&&E.length>=3){C=E[2]
}if(K.meta.referrer){S.set("r",C)}fsr$dbug.log("referrer: "+C);var D=this.decodeReferrer(document.referrer)||"";
if(K.meta.terms){S.set("st",D)}fsr$dbug.log("search terms: "+D)}this.processCPPDefaults(S)}this.v=G;
FSR.rid=S.get("rid");var F=K.tracker.timeout;if(K.tracker.adjust&&FSR.$defined(S.get("f"))){var R=S.get("to");
var B=((FSR.$time()-S.get("f"))/1000);fsr$dbug.log("ptimeout: "+R);fsr$dbug.log("loadtime: "+B);F=(0.9*R)+(0.1*(B*2));
F=Math.round(F*10)/10;if(F<2){F=2}else{if(F>5){F=5}}}if(K.tracker.adjust){S.set("to",F)}fsr$dbug.log("timeout: "+F);
fsr$dbug.log("invite: "+(FSR.$pick(S.get("i"),"")));fsr$dbug.log("tracker: "+(FSR.$pick(S.get("t")||"")));
...
for(var j=0,n=p.length;j<n;j++){var name=p[j].name;var value=p[j].value;if(name=="strIP"&&ipo==1){continue
...
}}return false},pool:function(){var C=new Date().getHours();var B=FSR.$P().pool||100;if(FSR.$type(B)!="array"){B=[{h:0,p:B}]
}var D=100;for(var A=0,E=B.length;A<E;A++){if(C>=B[A].h){D=B[A].p}}return D},invite:function(){var B=new Date().getTime();
var D=this.sd;FSR.$S.invite=0;FSR.c().set("i",FSR.$S.invite);this.setRepeatdays(1);if(FSR.$S.locale){FSR.c().set("l",FSR.$S.locale)
...
}}else{if(H.src=="navigator"){if(navigator.browserLanguage){A=navigator.browserLanguage}}}}}}A=A||"";
var C=H.locales||[];for(var G=0,E=C.length;G<E;G++){if(FSR.$type(C[G].match)!="array"){C[G].match=[C[G].match]
...
if(!B){return C}var C=unescape(B[1]);if(C){C=C.replace(/\+/g," ")}return C},setRepeatdays:function(E){var D=FSR.$P();
if(this.sd.repeatdays[E]){var B=new FSR.Hash.Cookie(FSR.cookieName("fsr.r"),{path:"/",domain:FSR.site.domain||false,duration:this.sd.repeatdays[E]});
B.set("d",this.sd.repeatdays[E]);if(D.altcookie&&D.altcookie.name){FSR.Cookie.write(D.altcookie.name,D.altcookie.value,{path:D.altcookie.path||false,domain:FSR.site.domain||false,duration:this.sd.repeatdays[E]})
...
}else{return C[1]}},cancelAlive:function(){clearInterval(fsr$timer);FSR.Cookie.dispose(FSR.cookieName("fsr.a"),{path:"/",domain:FSR.site.domain||false})
